...
async function createAuctionListing(
  ...
  try {
    const transaction = await marketplace?.auction.createListing({
      ...
      currencyContractAddress: NATIVE_TOKEN_ADDRESS, // NATIVE_TOKEN_ADDRESS is the cryptocurency that is native to the network. i.e. Polygon
  ...
  try {
    ...
    if (listing?.type === ListingType.Direct) {
      await marketplace?.direct.makeOffer(
        ...
        1, // Quantity = 1
        NATIVE_TOKENS[ChainId.Polygon].wrapped.address, // Polygon address
...
**Note:** For making offers, you'll need to have an ERC20 token. For our Polygon marketplace, that means you'll need to have Polygon/Matic.
